          ...
          <CForm class="row g-3">
            ...
            <div class="col-auto">
              <div class="row g-3 align-items-center bg-info" style="height:50px;">
                <div class="col-auto">
                  <label for="inputPassword6" class="col-form-label">Pencarian Nama Cabang</label>
                </div>
                <div class="col-auto">
        ...
        <CCardBody>
          <CDataTable hover striped :loading="loading" :items="items" :fields="fields">
            ...
            <template #button_td="{ item }">
              <td>
                <div v-if="item">
                  <CButton type="submit" @click="getModalEditForm(item)" size="sm" class="mr-2" color="success" v-if="AksesUpdate"> <i
                      class="cis-pencil"></i> <svg width="16" height="16" fill="currentColor" class="bi bi-trash"
                      viewBox="0 0 16 16">
                      <path
...
import ModalAddEdit from '@/views/ModalKantor/ModalAddEdit'
import ModalConfirm from '@/views/ModalKantor/ModalConfirm'
...
export default {
  ...
  methods: {
    ...
    MyfetchData(search = "") {
      fetch("http://localhost:8080/api/kantors/KantorData" + "?search=" + search)
        .then(res => res.json())
        .then(
          (result) => {
            0
    ...
    getModalAddForm() {
      ...
      this.storeDM.commit('changeTitleModal', { title: "Tambah Kantor Baru" })
    ...
    async getModalEditForm(item) {
      ...
      await this.storeDM.commit('changeTitleModal', { title: "Update Kantor" })
