    ...
    <CCol col="12" xl="12">
      <CCard>
        ...
        <CCardBody style="overflow: scroll">
          <CDataTable hover striped :loading="loading" :items="items" :fields="fields" style="width:2000px">
            ...
            <template #kantor="{ item }">
              <td>
                <div v-if="item">
                  <CBadge color="primary">{{ primaryBadge(item.kodeCabang + " - " + item.namaCabang) }}</CBadge>
                </div>
              </td>
            </template>
            <template #nama="{ item }">
...
export default {
  ...
  methods: {
    ...
    MyfetchData(search = "",criteriaSearch="") {
      fetch("http://localhost:8080/api/penggunas/PenggunaData" + "?search=" + search +"&criteria="+criteriaSearch)
        .then(res => res.json())
        .then(
          (result) => {
            0
